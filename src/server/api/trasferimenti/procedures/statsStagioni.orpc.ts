...
export const statsStagioniORPCProcedure = publicProcedure
  .route({ method: 'GET', path: '/trasferimenti/statsStagioni', summary: 'Statistiche stagioni di un giocatore' })
  .input(z.object({ idGiocatore: z.number() }))
  .handler(async ({ input }) => {
    const idGiocatore = +input.idGiocatore
    try {
