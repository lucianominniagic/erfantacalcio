...
export const championsBracketProcedure = publicProcedure.query(async () => {
  try {
    const tornei = await getTornei()

    const champions = tornei.filter(
      (t) => t.nome.toLowerCase() === 'champions',
    )

    const semifinaliAndandoTorneo = champions.find(
      (t) => t.gruppoFase?.toLowerCase() === 'semifinali andata',
    )
    const semifinaliRitornoTorneo = champions.find(
      (t) => t.gruppoFase?.toLowerCase() === 'semifinali ritorno',
    )
    const finaleTorneo = champions.find(
      (t) => t.gruppoFase?.toLowerCase() === 'finale',
    )
